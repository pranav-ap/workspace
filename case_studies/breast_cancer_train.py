...
def get_datasets():
    ...
    filtered_columns = filtered_columns['Numeric Variables'].values
    filtered_columns = list(map(lambda col: col.strip(), filtered_columns))

    if not filtered_columns:
        logger.error("No features passed the correlation threshold. Adjust the threshold or check data.")
        sys.exit(1)
...
def main():
    ...
    if hasattr(best_model, 'predict_proba'):
        ...
        y_pred_proba = np.zeros((X_test_transformed.shape[0], 2))  # Default if no probability output
